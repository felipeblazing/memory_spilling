...
int main() {
    benchmark_output::print_benchmark_header("cuDF Table Converter Performance Comparison");
    ...
    try {
        ...
        // ============================================================================
        benchmark_output::print_section_header("SINGLE-THREADED BENCHMARK");

        // Single-threaded: Convert to host memory
        std::cout << "\nSingle-threaded: Convert to host memory..." << std::endl;
        auto start_time = std::chrono::high_resolution_clock::now();
        
        auto single_table_alloc = spilling::cudf_table_converter::convert_to_host(
            table_view, custom_mr.get(), rmm::cuda_stream_view(default_stream));
        
        auto end_time = std::chrono::high_resolution_clock::now();
        auto single_convert_duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);
        
        double single_convert_throughput_mb_s = (target_size_mb * 1000000.0) / single_convert_duration.count();
        benchmark_output::print_conversion_results(
            "Conversion",
            single_table_alloc.allocation.size(),
            single_table_alloc.data_size,
            custom_mr->get_free_blocks(),
            single_convert_duration,
            single_convert_throughput_mb_s
        );

        // Single-threaded: Recreate from host memory
        std::cout << "\nSingle-threaded: Recreate from host memory..." << std::endl;
        start_time = std::chrono::high_resolution_clock::now();
        
        auto single_recreated_table = spilling::cudf_table_converter::recreate_table(
            single_table_alloc, rmm::cuda_stream_view(default_stream));
        
        end_time = std::chrono::high_resolution_clock::now();
        auto single_recreate_duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);
        
        double single_recreate_throughput_mb_s = (target_size_mb * 1000000.0) / single_recreate_duration.count();
        benchmark_output::print_recreation_results(
            "Recreation",
            single_recreated_table.num_columns(),
            single_recreated_table.num_rows(),
            single_recreate_duration,
            single_recreate_throughput_mb_s
        );


        // ============================================================================
        // CONCURRENT BENCHMARK (2 threads, 2 streams)
        // ============================================================================
        benchmark_output::print_section_header("CONCURRENT BENCHMARK (2 threads, 2 streams, separate data)");
        ...
        // Concurrent: Convert to host memory
        std::cout << "\nConcurrent: Convert to host memory (2 threads, separate data)..." << std::endl;
        start_time = std::chrono::high_resolution_clock::now();
        ...
        end_time = std::chrono::high_resolution_clock::now();
        auto concurrent_convert_duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);
        
        double concurrent_convert_throughput_mb_s = (target_size_mb * num_streams * 1000000.0) / concurrent_convert_duration.count();
        benchmark_output::print_concurrent_conversion_results(
            "Concurrent conversion",
            table_allocs[0].allocation.size(),
            table_allocs[0].data_size,
            custom_mr->get_free_blocks(),
            concurrent_convert_duration,
            concurrent_convert_throughput_mb_s
        );

        // Concurrent: Recreate from host memory
        std::cout << "\nConcurrent: Recreate from host memory (2 threads)..." << std::endl;
        start_time = std::chrono::high_resolution_clock::now();
        ...
        end_time = std::chrono::high_resolution_clock::now();
        auto concurrent_recreate_duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);
        
        double concurrent_recreate_throughput_mb_s = (target_size_mb * num_streams * 1000000.0) / concurrent_recreate_duration.count();
        benchmark_output::print_concurrent_recreation_results(
            ...
            concurrent_recreate_duration,
            concurrent_recreate_throughput_mb_s
        );
        ...
        // ============================================================================
        benchmark_output::print_section_header("PERFORMANCE COMPARISON SUMMARY");
        
        std::cout << "\nDataset size: " << target_size_mb << " MB per operation" << std::endl;
        std::cout << "Concurrent operations: " << num_streams << " streams" << std::endl;
        std::cout << "Total data processed (concurrent): " << (target_size_mb * num_streams) << " MB" << std::endl;
        
        std::cout << "\n" << std::string(80, '-') << std::endl;
        std::cout << std::setw(25) << "Operation" 
                  << std::setw(15) << "Single (ms)" 
                  << std::setw(15) << "Concurrent (ms)" 
                  << std::setw(15) << "Speedup" 
                  << std::setw(15) << "Efficiency" << std::endl;
        std::cout << std::string(80, '-') << std::endl;
        
        // Convert to host comparison
        double convert_speedup = static_cast<double>(single_convert_duration.count()) / concurrent_convert_duration.count();
        double convert_efficiency = convert_speedup / num_streams * 100.0;
        benchmark_output::print_performance_comparison_table(
            "Convert to host",
            single_convert_duration.count() / 1000.0,
            concurrent_convert_duration.count() / 1000.0,
            convert_speedup,
            convert_efficiency
        );
        
        // Recreate from host comparison
        double recreate_speedup = static_cast<double>(single_recreate_duration.count()) / concurrent_recreate_duration.count();
        double recreate_efficiency = recreate_speedup / num_streams * 100.0;
        benchmark_output::print_performance_comparison_table(
            "Recreate from host",
            single_recreate_duration.count() / 1000.0,
            concurrent_recreate_duration.count() / 1000.0,
            recreate_speedup,
            recreate_efficiency
        );
        ...
        std::cout << std::setw(25) << "Operation" 
                  << std::setw(20) << "Single (MB/s)" 
                  << std::setw(20) << "Concurrent (MB/s)" 
                  << std::setw(15) << "Improvement" << std::endl;
        std::cout << std::string(80, '-') << std::endl;
        
        benchmark_output::print_throughput_comparison_table(
            "Convert to host",
            single_convert_throughput_mb_s,
            concurrent_convert_throughput_mb_s,
            concurrent_convert_throughput_mb_s / single_convert_throughput_mb_s
        );
        ...
        benchmark_output::print_throughput_comparison_table(
            ...
            single_recreate_throughput_mb_s,
            concurrent_recreate_throughput_mb_s,
            concurrent_recreate_throughput_mb_s / single_recreate_throughput_mb_s
        );
        ...
        benchmark_output::print_success("Performance comparison completed successfully!");
